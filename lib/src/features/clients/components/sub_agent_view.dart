...
class SubAgentView extends StatelessWidget with GetItMixin {
  SubAgentView({super.key});
    ...
    return SizedBox(
      child: Padding(
        ...
        child: ListView.builder(
          itemCount: watchOnly((StockViewModel x) => x.subAgents).length,
          itemBuilder: (context, index) {
            var item = get<StockViewModel>().subAgents[index];
            return Padding(
              padding: const EdgeInsets.all(8.0),
              key: ValueKey(index + 4000),
              child: Card(
                child: InkWell(
                  onTap: () async {
                    ...
                    await get<StockViewModel>().getTableBySubAgentId(
                      context: context,
                      subAgentId: item['id'],
                    );
